...
int paired_main(int argc, char *argv[]) {
    ...
    while (1) {
        int option_index = 0;
        optc = getopt_long(argc, argv, "df:r:c:t:o:p:m:s:q:l:xng", paired_long_options, &option_index);
    ...
    if (infnc) {                /* using combined input file */
        ...
        if (!strcmp(infn1, infn2) || !strcmp(infn1, outfn1) || !strcmp(infn1, outfn2) ||
            !strcmp(infn1, sfn) || !strcmp(infn2, outfn1) || !strcmp(infn2, outfn2) || !strcmp(infn2, sfn) || !strcmp(outfn1, outfn2) || !strcmp(outfn1, sfn) || !strcmp(outfn2, sfn)) {
    ...
    while ((l1 = kseq_read(fqrec1)) >= 0) {
        ...
        if (p1cut->three_prime_cut >= 0 && p2cut->three_prime_cut >= 0) {
            if (!gzip_output) {
                ...
                if (pec) {
                    gzprintf(combo_gzip, "@%s", fqrec1->name.s);
                    if (fqrec1->comment.l) gzprintf(combo_gzip, " %s\n", fqrec1->comment.s);
                    else gzprintf(combo_gzip, "\n");
                    gzprintf(combo_gzip, "%.*s\n", p1cut->three_prime_cut - p1cut->five_prime_cut, fqrec1->seq.s + p1cut->five_prime_cut);
                    gzprintf(combo_gzip, "+\n");
                    gzprintf(combo_gzip, "%.*s\n", p1cut->three_prime_cut - p1cut->five_prime_cut, fqrec1->qual.s + p1cut->five_prime_cut);

                    gzprintf(combo_gzip, "@%s", fqrec2->name.s);
                    if (fqrec2->comment.l) gzprintf(combo_gzip, " %s\n", fqrec2->comment.s);
                    else gzprintf(combo_gzip, "\n");
                    gzprintf(combo_gzip, "%.*s\n", p2cut->three_prime_cut - p2cut->five_prime_cut, fqrec2->seq.s + p2cut->five_prime_cut);
                    gzprintf(combo_gzip, "+\n");
                    gzprintf(combo_gzip, "%.*s\n", p2cut->three_prime_cut - p2cut->five_prime_cut, fqrec2->qual.s + p2cut->five_prime_cut);
                } else {
                    gzprintf(outfile1_gzip, "@%s", fqrec1->name.s);
                    if (fqrec1->comment.l) gzprintf(outfile1_gzip, " %s\n", fqrec1->comment.s);
                    else gzprintf(outfile1_gzip, "\n");
                    gzprintf(outfile1_gzip, "%.*s\n", p1cut->three_prime_cut - p1cut->five_prime_cut, fqrec1->seq.s + p1cut->five_prime_cut);
                    gzprintf(outfile1_gzip, "+\n");
                    gzprintf(outfile1_gzip, "%.*s\n", p1cut->three_prime_cut - p1cut->five_prime_cut, fqrec1->qual.s + p1cut->five_prime_cut);

                    gzprintf(outfile2_gzip, "@%s", fqrec2->name.s);
                    if (fqrec2->comment.l) gzprintf(outfile2_gzip, " %s\n", fqrec2->comment.s);
                    else gzprintf(outfile2_gzip, "\n");
                    gzprintf(outfile2_gzip, "%.*s\n", p2cut->three_prime_cut - p2cut->five_prime_cut, fqrec2->seq.s + p2cut->five_prime_cut);
                    gzprintf(outfile2_gzip, "+\n");
                    gzprintf(outfile2_gzip, "%.*s\n", p2cut->three_prime_cut - p2cut->five_prime_cut, fqrec2->qual.s + p2cut->five_prime_cut);
                }
            }
        ...
        /* if only one sequence passed filter, then put its record in singles and discard the other */
            ...
            if (!gzip_output) {
                ...
                gzprintf(single_gzip, "@%s", fqrec1->name.s);
                if (fqrec1->comment.l) gzprintf(single_gzip, " %s\n", fqrec1->comment.s);
                else gzprintf(single_gzip, "\n");
                gzprintf(single_gzip, "%.*s\n", p1cut->three_prime_cut - p1cut->five_prime_cut, fqrec1->seq.s + p1cut->five_prime_cut);

                gzprintf(single_gzip, "+\n");
                gzprintf(single_gzip, "%.*s\n", p1cut->three_prime_cut - p1cut->five_prime_cut, fqrec1->qual.s + p1cut->five_prime_cut);
            }
            ...
            if (!gzip_output) {
                ...
                gzprintf(single_gzip, "@%s", fqrec2->name.s);
                if (fqrec2->comment.l) gzprintf(single_gzip, " %s\n", fqrec2->comment.s);
                else gzprintf(single_gzip, "\n");
                gzprintf(single_gzip, "%.*s\n", p2cut->three_prime_cut - p2cut->five_prime_cut, fqrec2->seq.s + p2cut->five_prime_cut);
